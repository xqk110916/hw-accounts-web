...
| 调拨依据查询 | `transferBasisQuery`| `transferBasisQuery`| 按调拨依据（调出）聚合查询物资的数量与重量，支持下钻查看容器明细列表 |
...
| 出库管理 | `outbound` | `outbound` | 办理物资调拨出库、消耗等离库作业流程；任务提交后直接进入待审核状态，不再使用前端确认操作 |
| 出库管理-自动配置 | `outbound` | `outbound` | 在添加、编辑出库任务时，由用户在自动配置弹窗维护一条或多条待出库物料条件，调用 `/busin/pick/auto-weight` 生成取货方案，并将选中方案回填出库明细 |
| 位置移动 | `move` | `move` | 处理库房内部特定位置间的转移、倒库作业；支持移库申请、审核、已审核数据修改、修改后审核和变更记录追溯 |
...
2. **后端注册手动触发**：
   - 页面代码或 JSON 修改完毕后，默认不自动调用 `.agent/skills/menu_management/SKILL.md`（菜单管理技能）。仅当用户明确要求注册菜单/权限，或明确要求触发菜单管理技能时，才使用该技能完成新模块在后端接口及数据库的权限与菜单节点注册。
