...
```
1. mcp__cdp-bridge__browser_get_tabs → 找到 http://127.0.0.1:8082/ 标签页
2. mcp__cdp-bridge__browser_execute_js → 获取认证 Token：
```
...
var tokenName = cookies['tokenName'];
var tokenValue = cookies['tokenValue'];
window.__authInfo = JSON.stringify({ tokenName: tokenName, tokenValue: tokenValue });
```

等待 `window.__authInfo !== undefined` 后读取结果。

**2. 封装统一的 API 调用函数：**

在后续所有操作中，使用以下 fetch 模板：
...
fetch('/base/{module}/{action}', {
  ...
  window.__apiResult = JSON.stringify(d);
});
```

**如果 Token 获取失败**，提示用户确保浏览器已打开系统页面（127.0.0.1:8082）并已登录。
