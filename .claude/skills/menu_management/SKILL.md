...
### 第二步：获取鉴权 Token (自动获取)
由于接口需要鉴权，Agent **可以且应当**尝试自动获取 Token。请求头为动态 key，即 `headers[tokenName] = tokenValue`（**不要**写死为 `Authorization`）：
...
---
*本技能由 Antigravity 自动维护，详见 [菜单管理-接口.md](file:///f:/testCode/hw-accounts-web/md/系统管理接口.md)*
