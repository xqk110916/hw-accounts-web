...
使用 MCP cdp-bridge 工具（详见 [system-admin 技能](../skills/system-admin/SKILL.md) 第零步）：
1. `mcp__cdp-bridge__browser_get_tabs` → 找到 `localhost:8080` 标签页，取 tab id
2. `mcp__cdp-bridge__browser_execute_js`（`switch_tab_id=<tab>`、`no_monitor=true`）→ 从 cookie 获取 tokenName 和 tokenValue
3. 后续 fetch 异步结果用 `mcp__cdp-bridge__browser_wait`（`condition_js="window.__xxx !== undefined"`）等待
