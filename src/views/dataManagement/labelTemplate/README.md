...
- 打印前会校验标签字段，随后通过 `Vue.prototype.$zplPrinter` 建立 WebSocket 连接并发送 ZPL 指令。

## 二维码生成

- **生成时机**：二维码由后端在保存标签数据时自动生成，前端不再动态生成二维码内容。
- **显示逻辑**：新增记录时显示”保存后生成”占位符；编辑/查看已有记录时显示后端返回的二维码图片（Base64 格式）。
- **打印处理**：打印时如后端未返回二维码，前端会基于当前表单数据临时生成二维码内容用于 ZPL 打印指令。
...
- 保存标签数据时，前端不再发送 `qrcodeBase64` 字段，由后端根据业务规则自动生成二维码。
