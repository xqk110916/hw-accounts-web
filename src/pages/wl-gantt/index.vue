  ...
  <div class="wl-gantt" id="wl-gantt">
    ...
    >
      <template v-if="!ganttOnly">
        ...
        <el-table-column v-if="useIndexColumn" fixed type="index" width="50" label="序号" align="center"></el-table-column>
        <template v-if="gantttype == 'schedule'">
          <el-table-column
            fixed
            label="任务阶段"
            min-width="140"
          ...
          >
            <template slot-scope="scope">
              <strong class="h-full">
                <span>
                  {{ nameFormatter ? nameFormatter(scope.row, scope.column, scope.treeNode, scope.$index) : scope.row[selfProps.name] }}
                </span>
              </strong>
            </template>
          </el-table-column>
          <el-table-column fixed min-width="140" label="任务标题" :prop="selfProps.week" align="center">
            <template slot-scope="scope">
              <div class="h-full">{{ scope.row.week }}</div>
            </template>
          </el-table-column>
          <el-table-column :resizable="false" fixed width="120" align="center" :prop="selfProps.startDate" label="计划开始">
            <template slot-scope="scope">
              <div class="h-full" @click="cellEdit('_s_d_' + scope.$index, 'wl-start-date')">
                {{ timeFormat(scope.row[selfProps.startDate]) }}
              </div>
            </template>
          </el-table-column>
          <el-table-column fixed :resizable="false" width="120" align="center" :prop="selfProps.endDate" label="计划完成">
            <template slot-scope="scope">
              <div class="h-full" @click="cellEdit('_e_d_' + scope.$index, 'wl-end-date')">
                {{ timeFormat(scope.row[selfProps.endDate]) }}
              </div>
            </template>
          </el-table-column>
          <el-table-column fixed min-width="60" label="计划天数" :prop="selfProps.today" align="center">
            <template slot-scope="scope">
              <div class="h-full">{{ scope.row.today }}</div>
            </template>
          </el-table-column>
          <el-table-column :resizable="false" fixed width="120" align="center" :prop="selfProps.realStartDate" label="实际开始">
            <template slot-scope="scope">
              <div class="h-full" @click="cellEdit('_s_d_' + scope.$index, 'wl-start-date')">
                {{ timeFormat(scope.row[selfProps.realStartDate]) }}
              </div>
            </template>
          </el-table-column>
          <el-table-column fixed :resizable="false" width="120" align="center" :prop="selfProps.realEndDate" label="实际完成">
            <template slot-scope="scope">
              <div class="h-full" @click="cellEdit('_e_d_' + scope.$index, 'wl-end-date')">
                {{ timeFormat(scope.row[selfProps.realEndDate]) }}
              </div>
            </template>
          </el-table-column>
          <el-table-column fixed min-width="60" label="实际耗时天数" :prop="selfProps.today" align="center">
            <template slot-scope="scope">
              <div class="h-full">{{ scope.row.today }}</div>
            </template>
          </el-table-column>
          <el-table-column fixed min-width="60" label="实际进度" :prop="selfProps.today" align="center">
            <template slot-scope="scope">
              <div class="h-full">{{ scope.row.today }}</div>
            </template>
          </el-table-column>
          <el-table-column fixed min-width="80" label="状态" :prop="selfProps.today" align="center">
            <template slot-scope="scope">
              <div class="h-full" :style="taskcolor(scope.row.today)">{{ taskStatusFilter(scope.row.today) }}</div>
            </template>
          </el-table-column>
        </template>
        <template v-if="gantttype == 'yeartype'">
          <el-table-column
            fixed
            label="所属部门"
            min-width="140"
          ...
          <el-table-column fixed min-width="140" label="项目名称" :prop="selfProps.week" align="center">
            <template slot-scope="scope">
              <div class="h-full">{{ scope.row.week }}</div>
            </template>
          </el-table-column>
          <el-table-column fixed min-width="120" label="项目经理" :prop="selfProps.week" align="center">
            <template slot-scope="scope">
              <div class="h-full">{{ scope.row.week }}</div>
            </template>
          </el-table-column>
          <el-table-column :resizable="false" fixed width="120" align="center" :prop="selfProps.startDate" label="项目开始日期">
            <template slot-scope="scope">
              <div class="h-full" @click="cellEdit('_s_d_' + scope.$index, 'wl-start-date')">
                {{ timeFormat(scope.row[selfProps.startDate]) }}
              </div>
          ...
          <el-table-column fixed :resizable="false" width="120" align="center" :prop="selfProps.endDate" label="预计完工日期">
            <template slot-scope="scope">
              <div class="h-full" @click="cellEdit('_e_d_' + scope.$index, 'wl-end-date')">
                {{ timeFormat(scope.row[selfProps.endDate]) }}
              </div>
          ...
          <el-table-column :resizable="false" fixed width="120" align="center" :prop="selfProps.realStartDate" label="实际完工日期">
            <template slot-scope="scope">
              <div class="h-full" @click="cellEdit('_s_d_' + scope.$index, 'wl-start-date')">
                {{ timeFormat(scope.row[selfProps.realStartDate]) }}
              </div>
          ...
          <el-table-column fixed min-width="60" label="实际投入天数" :prop="selfProps.today" align="center">
            <template slot-scope="scope">
              <div class="h-full">{{ scope.row.today }}</div>
            </template>
          </el-table-column>
        ...
        <template v-if="gantttype == 'moontype'">
          <el-table-column
            fixed
            label="所属部门"
            min-width="140"
          ...
          <el-table-column fixed min-width="140" label="项目类型" :prop="selfProps.week" align="center">
            <template slot-scope="scope">
              <div class="h-full">{{ scope.row.week }}</div>
            </template>
          </el-table-column>
          <el-table-column fixed min-width="120" label="项目名称" :prop="selfProps.week" align="center">
            <template slot-scope="scope">
              <div class="h-full">{{ scope.row.week }}</div>
            </template>
          </el-table-column>
          <el-table-column fixed min-width="120" label="项目任务" :prop="selfProps.week" align="center">
            <template slot-scope="scope">
              <div class="h-full">{{ scope.row.week }}</div>
            </template>
          </el-table-column>
      ...
      <template v-if="self_date_type === 'yearAndMonth'">
        <el-table-column :resizable="false" v-for="year in ganttTitleDate" :label="year.name" :key="year.id">
          <el-table-column class-name="wl-gantt-item" v-for="month in year.children" :resizable="false" :key="month.id" :label="month.name">
            <template slot-scope="scope">
      ...
      <!-- week and  Day gantt -->
      <template v-else-if="self_date_type === 'weekAndDay'">
        <el-table-column :resizable="false" v-for="i in ganttTitleDate" :label="i.week" :key="i.id">
          <el-table-column class-name="wl-gantt-item" :resizable="false" :label="i.name">
            <template slot-scope="scope">
              <div v-if="useRealTime" :class="realDayGanttType(scope.row, i.full_date)"></div>
              <div v-if="useRealTime" :class="haveDayGanttType(scope.row, i.full_date)"></div>
              <div :class="dayGanttType(scope.row, i.full_date)"></div>
            </template>
          </el-table-column>
        </el-table-column>
      </template>
    ...
    <!-- 组件区 -->
    <!-- 右键菜单 -->
    <context-menu :visible.sync="contextMenu.show" :x="contextMenu.x" :y="contextMenu.y" :menuList="contextMenu.data"></context-menu>
    <!-- hover 看板 -->
...
export default {
  ...
  computed: {
    ...
    ganttTitleDate() {
      ...
      //   }
      // }
      if (this.gantttype == 'moontype') {
        const date = new Date();
        const year = date.getFullYear();
        const month = date.getMonth() + 1; // 注意月份从0开始，需要加1
        return this.moontypeTitleDate(year, month, year, month);
      } else {
        // 不自动更新日期类型，以dateType固定展示
        if (this.self_date_type === 'yearAndWeek') {
          return this.yearAndWeekTitleDate(start_year, start_mouth, start_day, end_year, end_mouth, end_day);
        } else if (this.self_date_type === 'monthAndDay') {
          return this.mouthAndDayTitleDate(start_year, start_mouth, start_day, end_year, end_mouth, end_day);
        } else if (this.self_date_type === 'weekAndDay') {
          return this.weekAndDayTitleDate(start_year, start_mouth, start_day, end_year, end_mouth, end_day);
        } else {
          return this.yearAndMouthTitleDate(start_year, start_mouth, start_day, end_year, end_mouth, end_day);
        }
      }
    },
    // 数据
    ...
    selfProps() {
      return {
        ...
        haveStartDate: 'haveStartDate', //进行时间
        endDate: 'endDate', // 结束时间字段
        realEndDate: 'realEndDate', // 实际结束时间字段
        haveEndDate: 'haveEndDate', // 实际结束时间字段
  ...
  methods: {
     ...
     * end_mouth：结束月
     */
    yearAndMouthTitleDate(start_year, start_mouth, start_day, end_year, end_mouth, end_day) {
      // 日期数据盒子
      let dates = [
        {
          name: `${start_year}年`,
      ...
      if (year_diff === 0) {
        ...
        let mouths = this.generationMonths(start_year, start_mouth, start_day, end_mouth + 1, end_day, isLeap, false); // 处理月份
        dates[0].children = mouths;
        return dates;
      }
      // 处理开始月份
      let startIsLeap = this.isLeap(start_year);
      let start_mouths = this.generationMonths(start_year, start_mouth, start_day, 13, end_day, startIsLeap, false);
      // 处理结束月份
      let endIsLeap = this.isLeap(end_year);
      let end_mouths = this.generationMonths(end_year, 1, start_day, end_mouth + 1, end_day, endIsLeap, false);
      // 年间隔等于一年
      if (year_diff === 1) {
      ...
      if (year_diff > 1) {
        ...
        for (let i = 1; i < year_diff; i++) {
          ...
          let month_and_day = this.generationMonths(item_year, 1, start_day, 13, end_day, isLeap, false);
          dates.push({
            name: `${item_year}年`,
            date: item_year,
            id: uuidv4(),
     ...
     * end_mouth：结束月
     */
    yearAndWeekTitleDate(start_year, start_mouth, start_day, end_year, end_mouth, end_day) {
      // 处理年份
      let year_diff = end_year - start_year;
      // 只存在同年或前后年的情况
      if (year_diff === 0) {
        // 年间隔为同一年
        let isLeap = this.isLeap(start_year); // 是否闰年
        let mouths = this.generationMonths(start_year, start_mouth, start_day, end_mouth + 1, end_day, isLeap, true, true); // 处理月份
        return mouths;
      }
      // 处理开始月份
      let startIsLeap = this.isLeap(start_year);
      let start_mouths = this.generationMonths(start_year, start_mouth, start_day, 13, end_day, startIsLeap, true, true);
      // 处理结束月份
      let endIsLeap = this.isLeap(end_year);
      let end_mouths = this.generationMonths(end_year, 1, start_day, end_mouth + 1, end_day, endIsLeap, true, true);
     ...
     * end_mouth：结束月
     */
    mouthAndDayTitleDate(start_year, start_mouth, start_day, end_year, end_mouth, end_day) {
      // 处理年份
      let year_diff = end_year - start_year;
      // 只存在同年或前后年的情况
      if (year_diff === 0) {
        // 年间隔为同一年
        let isLeap = this.isLeap(start_year); // 是否闰年
        let mouths = this.generationMonths(start_year, start_mouth, start_day, end_mouth + 1, end_day, isLeap); // 处理月份
        return mouths;
      }
      // 处理开始月份
      let startIsLeap = this.isLeap(start_year);
      let start_mouths = this.generationMonths(start_year, start_mouth, start_day, 13, end_day, startIsLeap);
      // 处理结束月份
      let endIsLeap = this.isLeap(end_year);
      let end_mouths = this.generationMonths(end_year, 1, start_day, end_mouth + 1, end_day, endIsLeap);
      return start_mouths.concat(end_mouths);
    },
    /**
     * 生成月份函数
     ...
     * week: 是否以周的间隔
     */
    generationMonths(year, start_num = 1, start_day, end_num = 13, end_day, isLeap = false, insert_days = true, week = false) {
      let months = [];
      if (insert_days) {
        // 无需 日 的模式
        for (let i = start_num; i < end_num; i++) {
          ...
          let days = [];
          if (i == start_num) {
            days = this.generationDays(year, start_day, i, null, isLeap, week);
          } else if (i == end_num - 1) {
            days = this.generationDays(year, null, i, end_day, isLeap, week);
          } else {
            days = this.generationDays(year, null, i, null, isLeap, week);
          }
          months.push({
            name: `${i}月`,
            date: i,
     ...
     * week: Boolean 是否间隔一周
     */
    generationDays(year, start_day, month, end_day, isLeap = false, week = false) {
      ...
      if (week) {
        ...
        for (let i = start_days; i < end_days; i++) {
          days.push({
            date: i,
            name: `${i}日`,
            id: uuidv4(),
     ...
     * end_mouth：结束月
     */
    moontypeTitleDate(start_year, start_mouth, end_year, end_mouth) {
      // 处理年份
      let year_diff = end_year - start_year;
      // 只存在同年或前后年的情况
      if (year_diff === 0) {
        // 年间隔为同一年
        let isLeap = this.isLeap(start_year); // 是否闰年
        let mouths = this.generationmoontype(start_year, start_mouth, end_mouth + 1, isLeap); // 处理月份
        return mouths;
      }
      // 处理开始月份
      let startIsLeap = this.isLeap(start_year);
      let start_mouths = this.generationmoontype(start_year, start_mouth, 13, startIsLeap);
      // 处理结束月份
      let endIsLeap = this.isLeap(end_year);
      let end_mouths = this.generationmoontype(end_year, 1, end_mouth + 1, endIsLeap);
      return start_mouths.concat(end_mouths);
    },
    /**
     * 月-日模式gantt标题
     ...
     * end_mouth：结束月
     */
    weekAndDayTitleDate(start_year, start_mouth, start_day, end_year, end_mouth, end_day) {
      // 处理年份
      let year_diff = end_year - start_year;
      // 只存在同年或前后年的情况
      if (year_diff === 0) {
        // 年间隔为同一年
        let isLeap = this.isLeap(start_year); // 是否闰年
        let mouths = this.generationMonthlist(start_year, start_mouth, start_day, end_mouth + 1, end_day, isLeap); // 处理月份
        return mouths;
      }
      // 处理开始月份
      let startIsLeap = this.isLeap(start_year);
      let start_mouths = this.generationMonthlist(start_year, start_mouth, start_day, 13, end_day, startIsLeap);
      // 处理结束月份
      let endIsLeap = this.isLeap(end_year);
      let end_mouths = this.generationMonthlist(end_year, 1, start_day, end_mouth + 1, end_day, endIsLeap);
      return start_mouths.concat(end_mouths);
    },
    /**
     * 生成月份函数
     ...
     * week: 是否以周的间隔
     */
    generationMonthlist(year, start_num = 1, start_day, end_num = 13, end_day, isLeap = false, insert_days = true, week = false) {
      let months = [];
      let monthlist = [];
      if (insert_days) {
        // 无需 日 的模式
        for (let i = start_num; i < end_num; i++) {
          ...
          let weeklist = [];
          if (i == start_num) {
            days = this.generationDaylist(year, start_day, i, null, isLeap, week);
            weeklist = this.generationweeklist(year, start_day, i, null, isLeap);
          } else if (i == end_num - 1) {
            days = this.generationDaylist(year, null, i, end_day, isLeap, week);
            weeklist = this.generationweeklist(year, null, i, end_day, isLeap, week);
          } else {
            days = this.generationDaylist(year, null, i, null, isLeap, week);
            weeklist = this.generationweeklist(year, null, i, null, isLeap, week);
          }
          months.push({
            name: `${i}月`,
            date: i,
        ...
        for (let i = 0; i < months.length; i++) {
          monthlist = monthlist.concat(months[i].children);
        }
    ...
    generationmoontype(year, start_num = 1, end_num = 13, isLeap = false, insert_days = true, week = false) {
      let months = [];
      let monthlist = [];
      if (insert_days) {
        // 无需 日 的模式
        for (let i = start_num; i < end_num; i++) {
          // 需要 日 的模式
          let days = this.generationmoon(year, i, isLeap, week);
     ...
     * week: Boolean 是否间隔一周
     */
    generationmoon(year, month, isLeap = false, week = false) {
      let big_month = [1, 3, 5, 7, 8, 10, 12].includes(month);
      let small_month = [4, 6, 9, 11].includes(month);
      let dates_num = big_month ? 32 : small_month ? 31 : isLeap ? 30 : 29;
      let days = [];
      if (week) {
        ...
        let start_days = 1;
        for (let i = start_days; i < dates_num; i++) {
          let _start_day_inweek = this.timeInWeek(`${year}-${month}-${i}`);
          let weeks = '一';
          if (_start_day_inweek == 0) {
            weeks = '日';
          } else if (_start_day_inweek == 1) {
            weeks = '一';
          } else if (_start_day_inweek == 2) {
            weeks = '二';
          } else if (_start_day_inweek == 3) {
            weeks = '三';
          } else if (_start_day_inweek == 4) {
            weeks = '四';
          } else if (_start_day_inweek == 5) {
            weeks = '五';
          } else if (_start_day_inweek == 6) {
            weeks = '六';
          }
          days.push({
            date: i,
     ...
     * week: Boolean 是否间隔一周
     */
    generationDaylist(year, start_day, month, end_day, isLeap = false, week = false) {
      let big_month = [1, 3, 5, 7, 8, 10, 12].includes(month);
      let small_month = [4, 6, 9, 11].includes(month);
      let dates_num = big_month ? 32 : small_month ? 31 : isLeap ? 30 : 29;
      let days = [];
    ...
    generationweeklist(year, start_day, month, end_day, isLeap = false) {
      let big_month = [1, 3, 5, 7, 8, 10, 12].includes(month);
      let small_month = [4, 6, 9, 11].includes(month);
      let dates_num = big_month ? 32 : small_month ? 31 : isLeap ? 30 : 29;
      let weeklist = [];
    ...
    dayGanttType(row, date, unit = 'days') {
      let start_date = null;
      let end_date = null;
      if (row[this.selfProps.startDate] !== undefined) {
        start_date = row[this.selfProps.startDate];
      }
      if (row[this.selfProps.endDate] !== undefined) {
        end_date = row[this.selfProps.endDate];
      }
    ...
    realDayGanttType(row, date, unit = 'days') {
      let start_date = null;
      let end_date = null;
      if (row[this.selfProps.realStartDate] !== undefined) {
        start_date = row[this.selfProps.realStartDate];
      }
      if (row[this.selfProps.realEndDate] !== undefined) {
        end_date = row[this.selfProps.realEndDate];
      }
      let between = dayjs(date).isBetween(start_date, end_date, unit);
      if (between) {
        return 'wl-real-on';
     ...
     * unit: string 时间单位，以天、月、年计算
     */
    haveDayGanttType(row, date, unit = 'days') {
      let start_date = null;
      let end_date = null;
      if (row[this.selfProps.haveStartDate] !== undefined) {
        start_date = row[this.selfProps.haveStartDate];
      }
      if (row[this.selfProps.haveEndDate] !== undefined) {
        end_date = row[this.selfProps.haveEndDate];
      }
      let between = dayjs(date).isBetween(start_date, end_date, unit);
      if (between) {
        return 'wl-have-on';
    ...
    parentStartDateToChild(item) {
      ...
      let _child_early_parent = this.timeIsBefore(item[this.selfProps.startDate], item._parent[this.selfProps.startDate]);
    ...
    targetLinkLoopback(item, pre_tesk = []) {
      ...
      if (this.preMultiple) {
        ...
        _legal_pres.forEach(i => {
          let _pre_target = this.self_data_list.find(t => t[this.selfProps.id] === i);
          if (_pre_target && Array.isArray(_pre_target[this.selfProps.pre]) && _pre_target[this.selfProps.pre].length > 0) {
